...
int main () {
    ...
    for (const auto& c : cnt) {
        if (cnt.count(c.second) == 0) {
            cnt.insert(make_pair(c.second, 1));
        } else {
            cnt.at(c.second)++;
        }
        cnt_vec.pb(c.second);
    ...
    auto c = comb(cnt_vec.size(), 3);
    for (const auto& v : c) {
        // print(v);
        int a = keys.at(v.at(0)-1);
        int b = keys.at(v.at(1)-1);
        int c = keys.at(v.at(2)-1);
        res += cnt.at(a) * cnt.at(b) * cnt.at(c);
    }
